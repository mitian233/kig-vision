"""A tiny compatibility shim that exposes a subset of the picamera API
implemented on top of linuxpy's video device.

This is intentionally minimal: it implements the API surface used by the
project's `main.py`. It is NOT a full replacement for picamera. Behavior is
best-effort and intended to let existing code run unchanged on systems where
`linuxpy` and a V4L2 camera are available.
"""
